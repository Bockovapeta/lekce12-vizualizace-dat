    ...
    .then(variables => {
        fetch(`https://onemocneni-aktualne.mzcr.cz/api/v3/zakladni-prehled?apiToken=${variables.apiToken}`)
            ...
            .then(data => {
                ...
                let podil = ((zakladniInfo.potvrzene_pripady_65_vcerejsi_den * 100) / zakladniInfo.potvrzene_pripady_vcerejsi_den)
                console.log(podil);
                document.getElementById('js-nakazeni65').innerText = (Math.round(podil * 100) / 100) + " %"
                let datumNakazenych65 = new Date(zakladniInfo.potvrzene_pripady_65_vcerejsi_den_datum);
                document.getElementById('js-nakazeni65-datum').innerText = datumNakazenych65.toLocaleDateString('cs-CZ');



                let potvrzenePripadyCelkem = zakladniInfo.potvrzene_pripady_celkem
                let celkemPotvrzeni = zakladniInfo.aktivni_pripady + zakladniInfo.vyleceni + zakladniInfo.umrti



                if (potvrzenePripadyCelkem === celkemPotvrzeni) {
                    console.log("Potvrzené případy celkem se rovnají součtu aktivních nemocných, vyléčených a zemřelých");
                }
